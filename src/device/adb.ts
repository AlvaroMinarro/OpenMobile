...
export class AdbWrapper {
  ...
  private async shell(serial: string, ...args: string[]): Promise<string> {
    return this.exec(["adb", "-s", serial, "shell", ...args]);
  }
  ...
  async inputTap(serial: string, x: number, y: number): Promise<void> {
    await this.shell(serial, "input", "tap", String(x), String(y));
  }
  ...
  async screencap(serial: string, localPath: string): Promise<void> {
    ...
    await this.shell(serial, "screencap", "-p", devicePath);
    await this.exec(["adb", "-s", serial, "pull", devicePath, localPath]);
  }
  ...
  async uiautomatorDump(serial: string): Promise<string> {
    ...
    await this.shell(serial, "uiautomator", "dump", devicePath);
    return this.shell(serial, "cat", devicePath);
  }
}
